...
import { getKeKhaiTaiSan } from '@networking/user';
import { translate } from '@utils/i18n/translate';
import { Box, FlatList } from 'native-base';

import ItemDotKeKhai from './Items/ItemDotKeKhai';
import styles from './styles';
import { DotKhaiBaoProps } from './type';

const KeKhaiTaiSan = () => {
  const [listDon, setListDon] = useState<DotKhaiBaoProps[]>([]);
  ...
  const getData = async () => {
    ...
    try {
      ...
      const res: any = await getKeKhaiTaiSan(body);
  ...
  return (
    <Box flex={1} backgroundColor={R.colors.backgroundColorNew}>
      <HeaderReal title={translate('slink:Asset_declaration')} />
      <FlatList
        data={listDon}
        extraData={listDon}
        onRefresh={getData}
        ...
        renderItem={({ item, index }) => (
          <ItemDotKeKhai item={item} key={index} refreshData={getData} />
        )}
      />
    </Box>
  );
