...
export function getMarkedDates(data: { title: string; data: any[] }[]) {
  const marked: any = {};
...
export function findClosestIndexGreaterThan(arr: number[], num: number) {
  const listNumber: any[] = [];

  const listSort = customSort(arr);

  for (let i = 0; i < listSort.length; i++) {
    if (listSort[i] <= num + 10) {
      listNumber.push(i);
    }
  }
