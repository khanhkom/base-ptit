...
import { getViTriChucDanh } from '@networking/user';
import { translate } from '@utils/i18n/translate';
import moment from 'moment';

const ViTriTuyenDung = ({ visibleEdit, infoUser, onShowDetail }: any) => {
  ...
  const getData = async () => {
    ...
    try {
      ...
      const responViTri = await getViTriChucDanh(body);
  ...
  const handleShowdata = (item: any) => {
    if (visibleEdit) {
      navigateScreen(APP_SCREEN.VITRICHUCDANHTABLE, {
        idUser: infoUser?._id,
        onRefresh: getData,
        item,
      });
  ...
  if (loading) {
    return (
      <BoxHSNS
        ...
        title={translate('slink:Position_title')}
        onPress={() => goToAdd()}>
        <SkeletonTable />
      </BoxHSNS>
    );
...
const listdata = (item: any) => [
  ...
  },
  {
    ...
    value: item?.laDonViChinh ? '✅' : '❌',
  },
  {
    label: translate('hoSoNhanSu:isBoMonChinh'),
    value: item?.isBoMonChinh ? '✅' : '❌',
  },
  {
