...
import { getTinHoc } from '@networking/user';
...
const TinHoc = ({
  ...
  const getData = async () => {
    try {
      ...
      const res = await getTinHoc(body);
  ...
  const goToAdd = () => {
    navigateScreen(APP_SCREEN.TINHOCTABLE, {
      idUser,
      onRefresh: getData,
    });
  };
  ...
  const tableData =
    listHocHam?.map((itemSV: any, indexSV: number) => {
      const dataRow = [
        ...
        <ItemInfor
          onPress={() => handleShow(itemSV)}
          content={itemSV?.tinHoc?.trinhDoTinHoc ?? '--'}
          key={indexSV}
        />,
        <ItemInfor
          onPress={() => handleShow(itemSV)}
  ...
  return (
    <BoxHSNS
      ...
      title={translate('hoSoNhanSu:tinHoc')}
      onPress={() => goToAdd()}>
      {loading ? (
        <SkeletonTable />
      ) : listHocHam?.length > 0 ? (
...
const listdata = (item: any) => [
  ...
  {
    label: translate('hoSoNhanSu:trinhDoTinHoc'),
    value: item?.tinHoc?.trinhDoTinHoc ?? '--',
  },
  {
