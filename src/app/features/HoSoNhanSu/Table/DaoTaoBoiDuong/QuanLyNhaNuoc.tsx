...
import { getQuanLyNhaNuoc } from '@networking/user';
...
const QuanLyNhaNuoc = ({
  ...
  const getData = async () => {
    try {
      ...
      const res = await getQuanLyNhaNuoc(body);
  ...
  const goToAdd = () => {
    navigateScreen(APP_SCREEN.QUANLYNHANUOCTABLE, {
      idUser,
      onRefresh: getData,
    });
  };
  ...
  const tableData =
    listHocHam?.map((itemSV: any, indexSV: number) => {
      const dataRow = [
        ...
        <ItemInfor
          onPress={() => handleShow(itemSV)}
          content={itemSV?.trinhDoQuanLyNhaNuoc?.ten ?? '--'}
          key={indexSV}
        />,
        <ItemInfor
          onPress={() => handleShow(itemSV)}
  ...
  return (
    <BoxHSNS
      ...
      title={translate('hoSoNhanSu:trinhDoQuanLyNhaNuoc')}
      onPress={() => goToAdd()}>
      {loading ? (
        <SkeletonTable />
      ) : listHocHam?.length > 0 ? (
...
const listdata = (item: any) => [
  ...
  {
    label: translate('hoSoNhanSu:trinhDoQuanLyNhaNuoc'),
    value: item?.trinhDoQuanLyNhaNuoc?.ten ?? '--',
  },
  {
