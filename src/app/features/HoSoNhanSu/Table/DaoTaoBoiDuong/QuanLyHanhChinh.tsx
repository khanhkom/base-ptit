...
import { getQuanLyHanhChinh } from '@networking/user';
import { translate } from '@utils/i18n/translate';
import moment from 'moment';

const QuanLyHanhChinh = ({
  onShowDetail,
  editVisible,
  idUser,
}: {
  ...
  const getData = async () => {
    try {
      ...
      const res = await getQuanLyHanhChinh(body);
  ...
  const goToAdd = () => {
    navigateScreen(APP_SCREEN.QUANLYHANHCHINHTABLE, {
      idUser,
      onRefresh: getData,
    });
  };
  ...
  const tableData =
    listHocHam?.map((itemSV: any, indexSV: number) => {
      const dataRow = [
        ...
        <ItemInfor
          onPress={() => handleShow(itemSV)}
          content={itemSV?.trinhDoQuanLyHanhChinh?.ten ?? '--'}
          key={indexSV}
        />,
        <ItemInfor
          onPress={() => handleShow(itemSV)}
  ...
  return (
    <BoxHSNS
      ...
      title={translate('hoSoNhanSu:trinhDoQuanLyHanhChinh')}
      onPress={() => goToAdd()}>
      {loading ? (
        <SkeletonTable />
      ) : listHocHam?.length > 0 ? (
...
export default QuanLyHanhChinh;
...
const listdata = (item: any) => [
  ...
  {
    label: translate('hoSoNhanSu:trinhDoQuanLyHanhChinh'),
    value: item?.trinhDoQuanLyHanhChinh?.ten ?? '--',
  },
  {
