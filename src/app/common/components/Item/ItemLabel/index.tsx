...
import { getFontSize, getLineHeight, HEIGHT, showLink, WIDTH } from '@common';
...
const ItemLabel = (props: ItemProps) => {
  const {
    nullItem,
    style,
    label,
  ...
  const openURL = () => {
    onPress ? onPress() : handleSeeDocument(link ?? '');
  };
          ...
          {typeHTML && isHTML(value) ? (
            ...
            <>
              {badge ? (
                ...
                <Text
                  disabled={!(onPress || !!link)}
                  onPress={openURL}
                  style={[
                    styles.textValue,
                    !multiLine && { textAlign: 'right' },
                    value === translate('slink:Chua_cap_nhat') &&
                      styles.txtChuaCapNhat,
                    (onPress || !!link) && styles.textLink,
                    textValue,
                  ]}>
