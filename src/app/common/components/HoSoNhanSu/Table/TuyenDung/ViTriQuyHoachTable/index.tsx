...
const ViTriQuyHoachTable = props => {
  const {
    control,
    handleSubmit,
    setValue,
  ...
  const onSubmit = async (data: any) => {
    ...
    let res: any;
    if (defaultData?._id) {
      res = await putViTriQuyHoach(body, defaultData?._id);
    } else {
      res = await postViTriQuyHoach(body);
    }
  ...
  const onDel = async () => {
    ...
    const res = await delViTriQuyHoach(defaultData?._id);
  ...
  if (loading) {
    return (
      <Box flex={1} backgroundColor={R.colors.backgroundColorNew}>
        <HeaderReal title="Vị trí, chức danh quy hoạch" />
        <Box flex={1}>
          <LoadingComponent />
        </Box>
      </Box>
...
export default ViTriQuyHoachTable;
