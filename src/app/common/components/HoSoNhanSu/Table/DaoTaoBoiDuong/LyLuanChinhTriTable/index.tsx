...
const LyLuanChinhTriTable = props => {
  const {
    control,
    handleSubmit,
    setValue,
  ...
  const getData = async () => {
    ...
    const responseAPILLCT = await getDSTrinhDoLyLuan();
  ...
  const onSubmit = async (data: any) => {
    ...
    const body = {
      ...
      trinhDoLyLuanChinhTriId: data?.trinhDoLyLuanChinhTriId || null,
    ...
    let res: any;
    if (defaultData?._id) {
      res = await putLyLuanChinhTri(body, defaultData?._id);
    } else {
  ...
  const onDel = async () => {
    ...
    const res = await delLyLuanChinhTri(defaultData?._id);
  ...
  if (loading) {
    return (
      <Box flex={1} backgroundColor={R.colors.backgroundColorNew}>
        <HeaderReal title="Trình độ lý luận chính trị" />
        <Box flex={1}>
          <LoadingComponent />
        </Box>
      </Box>
    ...
    <Box flex={1} backgroundColor={R.colors.backgroundColorNew}>
      ...
      <KeyboardAwareScrollView contentContainerStyle={styles.content}>
        ...
        <SingleSelectForm
          ...
          defaultValue={defaultData?.trinhDoLyLuanChinhTriId}
          data={listLLCT}
          name={'trinhDoLyLuanChinhTriId'}
          control={control}
          error={errors?.trinhDoLyLuanChinhTriId?.message}
          required
        />
        <InputNBForm
...
export default LyLuanChinhTriTable;
