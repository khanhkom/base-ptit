...
const TinHocTable = props => {
  ...
  const getData = async () => {
    ...
    const responseAPILLCT = await getListTinHoc();
  ...
  const onSubmit = async (data: any) => {
    ...
    let res: any;
    if (defaultData?._id) {
      res = await putTinHoc(body, defaultData?._id);
    } else {
  ...
  const onDel = async () => {
    ...
    const res = await delTinHoc(defaultData?._id);
    ...
    <Box flex={1} backgroundColor={R.colors.backgroundColorNew}>
      ...
      <KeyboardAwareScrollView contentContainerStyle={styles.content}>
        ...
        <SingleSelectForm
          ...
          defaultValue={defaultData?.tinHocId}
          data={listLLCT}
          name={'tinHocId'}
          control={control}
          error={errors?.tinHocId?.message}
          required
        />
        <InputNBForm
