...
const QuanLyHanhChinhTable = props => {
  const {
    control,
    handleSubmit,
    setValue,
  ...
  const getData = async () => {
    ...
    const responseAPILLCT = await getDSTrinhDoQuanLyHC();
  ...
  const onSubmit = async (data: any) => {
    ...
    const body = {
      ...
      trinhDoQuanLyHanhChinhId: data?.trinhDoQuanLyHanhChinhId || null,
    ...
    let res: any;
    if (defaultData?._id) {
      res = await putQuanLyHanhChinh(body, defaultData?._id);
    } else {
  ...
  const onDel = async () => {
    ...
    const res = await delQuanLyHanhChinh(defaultData?._id);
  ...
  if (loading) {
    return (
      <Box flex={1} backgroundColor={R.colors.backgroundColorNew}>
        <HeaderReal title="Trình độ quản lý hành chính" />
        <Box flex={1}>
          <LoadingComponent />
        </Box>
      </Box>
  ...
  return (
    <Box flex={1} backgroundColor={R.colors.backgroundColorNew}>
      <HeaderReal
        title="Trình độ quản lý hành chính"
      ...
      <KeyboardAwareScrollView contentContainerStyle={styles.content}>
        ...
        />
        <SingleSelectForm
          label="Trình độ quản lý hành chính"
          defaultValue={defaultData?.trinhDoQuanLyHanhChinhId}
          data={listLLCT}
          name={'trinhDoQuanLyHanhChinhId'}
          control={control}
          error={errors?.trinhDoQuanLyHanhChinhId?.message}
          required
        />
        <InputNBForm
...
export default QuanLyHanhChinhTable;
